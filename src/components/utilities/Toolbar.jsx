...
const Toolbar = ({handleAddColumn, handleAddRow, handleRemoveColumn, handleRemoveRow, isSmallScreen, handleDeleteGrid, setIsDrawing, handleModeChange, isCanvasMode, wasDeleted}) => {
    ...
    return (
        <div style={{color: 'white'}}>
          <Card sx={{bgcolor: "rgba(0, 0, 0, 0.5)", height: '100%'}}>
            <Grid container spacing={{ xs: 1, md: 2}}  columns={{ xs: 1}} sx={{justifyContent: 'spread-between'}}>
              <lay.Title isSmallScreen={isSmallScreen} heading="Grid Builder" />

              {/* ROWS */}
              <util.RowControl handleIncrementRow={handleIncrementRow} handleDecrementRow={handleDecrementRow} rowCount={rowCount}/>
  
              {/* COLUMNS */}
              <util.ColumnControl handleIncrementColumn={handleIncrementColumn} handleDecrementColumn={handleDecrementColumn} columnCount={columnCount}/>
        ...
        <Grid item>
          <Button variant="outlined" color="error" sx={{bgcolor: 'white', width:'90px', transform:'translate(60px,20px)', height:"40px"}} onMouseEnter={() => setIsDeleteHovered(true)}
            ...
            <Grid item>
              <FormGroup>
              <FormControlLabel control={<util.OrangeSwitch onChange={handleModeChange}/>} label={<Typography variant="subtitle1" color='white'>{isCanvasMode ? 'Canvas Mode' : 'Grid Mode'}</Typography>} />
              </FormGroup>
            </Grid>
        </Grid>
        </Grid>
